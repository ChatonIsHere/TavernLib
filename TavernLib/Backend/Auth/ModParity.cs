...
public static class ModParity
{
    ...
    /// <summary>
    /// Every server mod with ClientSide true is required of the joining
    /// client at that EXACT version - a build within the same major can still
    /// diverge in ways that break a shared session, so "close enough" isn't
    /// good enough. clientMods is id -> version, parsed from the client's own
    /// claim (whatever its Mods/ currently has enabled); a client with no
    /// claim at all is treated as having nothing, so every required mod
    /// mismatches rather than silently passing. Server-only mods (ClientSide
    /// false) are never checked here - the client's own extra client-only
    /// mods are its own business. Returns the mods the client is missing or
    /// holding at the wrong version; empty means parity holds.
    /// </summary>
    public static List<RequiredMod> ValidateClient(List<ModHandshake.Entry> serverMods, Dictionary<string, string> clientMods)
    {
        ...
        foreach (var mod in serverMods.Where(m => m.ClientSide))
        {
