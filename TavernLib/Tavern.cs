        ...
        try
        {
            ...
            if (CommandLineArguments.Contains(CommandLineArguments.StartServerArgument))
            {
                ...
                if (!isLauncherManaged && CommandLineArguments.TryGetNextArguments(TavernArgs.ModsListFile, 1, out var modListArgs))
                    ...
                    TavernLogger.Warn("mod reconcile: /modlist was passed but this server is launcher-managed (/launcherauth present); native mod reconciliation only runs for a genuinely headless server, so it was skipped. This launcher's own Python modmanager.py manages Mods/ for this server instead.");
            }
